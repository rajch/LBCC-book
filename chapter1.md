...
In our case, the CLR, which after all stands for the Common Language Runtime, will take care of runtime errors. In fact, the error messages that we saw in our two error cases were generated by the CLR.

An interesting note here. As the name suggests, the CLR is: a run-time environment common to many languages, including C#, Visual Basic.NET, F#, Boo, IronPython and so on. When the CLR was first released in the early 2000s, there was this in-joke among Visual Basic enthusiasts: the new Visual Basic runtime was so good, they decided to share it with other languages, and so the CLR was born. But I digress.
